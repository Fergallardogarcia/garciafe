...
def train_mixed_data(
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(trainloader):
            ...
            loss = criterion(outputs, labels)
            loss = torch.where(is_real, -alpha*loss, loss) # include alpha = num_total_samples/len(trainset) to correct for dtaset size strngth. now 1:1
            loss = loss.mean()  # Average over real samples
            loss.backward()
            optimizer.step()
            # print statistics
            running_loss += loss.item()
