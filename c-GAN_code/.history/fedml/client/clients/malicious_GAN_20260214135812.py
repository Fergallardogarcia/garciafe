...
class GanMaliciousClient(HonestClient):
    ...
    def generate_D_syn(self, updated_generator_model: torch.Tensor, server_round: int, trainset: Dataset, generate_config: Optional[Dict]) -> Dataset:
        """ 1. Generate synthetic data for client according to trainset distribution:
            ...
            2. Also, generate either:
                ...
                b. Many more, later adjusting per sample loss to account for this.  

            generate_config: 
                "D_syn_size": Int (Number) or Str ("Train_SIZE")
                "Proportion_per_class": Str ("Equal") or ("Train_like")
                
        """ 
        return
    
    def combined_loss(self, D_synth_criterion: torch.Tensor, D_real_criterion: torch.Tensor, combine_config: Optional[Dict]) -> torch.Tensor:
        ...
        g_loss_syn = D_synth_criterion(dis_predict, input_l).to(device)
        g_loss_real = D_real_criterion(dis_predict_real, input_l_real).to(device)
        combined_loss = self.combined_loss(g_loss_syn, g_loss_real, self.combine_config)
