...
class GanMaliciousClient(HonestClient):
    ...
    def __init__(
        ...
        if gan_model is None:
            self.malicious_generator = load_model(attack_config["GAN_ATTACK_CONFIG"]["HYPER_PARAM"]["GEN_ARGS"], model_name=filter_param["GEN_MODEL"])
        else:
            self.gan_model = gan_model
            GAN_ATTACK_CONFIG:                                  # GAN_ATTACK specific attack configs
            HYPER_PARAM:                
                SKIP_ROUNDS: 20
                GEN_ARGS:
                    MODEL_NAME: TEST-TANH                   # Options: [TEST-TANH, TEST-SIGMOID, GEN-DCGAN]
                    TRAIN: True                             # Not implemented in original code
                    SET_WEIGHTS: False                      #Probably can be fused with weight path and have the option null
                    WEIGHT_PATH: ./temp/weights/gen_weights
                    LATENT_SIZE: 100                      
                    NUM_CLASSES: 10                         #Can be removed as it is already the same as in filter configs
                    # OUT_CHANNEL: 3                          # Options: [MNIST=1, CIFAR-10=3]
