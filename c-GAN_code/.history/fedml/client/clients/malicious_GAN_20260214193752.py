...
class GanMaliciousClient(HonestClient):
    ...
    def generate_D_syn(
            ...
            device          ,#= self.device,
            batch_size = 1024, # Adjust to have same number of batches
            server_round: int,
            trainset: Dataset, 
        ...
        gen_config= attack_config["GAN_ATTACK_CONFIG"]["HYPER_PARAM"]["GEN_ARGS"]
        
        input_znoises = torch.randn(self.num_classes * self.samples_per_class, self.latent_size)

        input_classes = torch.arange(self.num_classes, dtype=torch.int64).repeat_interleave(self.samples_per_class)
