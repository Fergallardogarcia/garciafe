...
class GanMaliciousClient(HonestClient):
    ...
    def generate_D_syn(
        ...
        num_samples = gen_configs["SYTHETIC_DATA_SIZE"]


        input_znoises = torch.randn(num_samples, latent_size)

        input_classes = torch.arange(num_samples) % num_classes # By chatgpt
        ...
        device=device,
        batch_size=batch_size, # Adress later
    )
        alpha= num_samples / len(trainset) # Adjusting for different dataset sizes, if needed. If we generate the same number of samples as the trainset, alpha will be 1 and won't affect the loss. If we generate more samples than the trainset, alpha will be greater than 1 and will increase the loss, making the attack stronger. If we generate fewer samples than the trainset, alpha will be less than 1 and will decrease the loss, making the attack weaker.
