...
class GanMaliciousClient(HonestClient):
    ...
    def generate_D_syn(
        ...
        data_X_syn, data_Y_syn = gen_dataset.tensors
        data_X_real, data_Y_real = trainset.tensors

        is_generated_syn = torch.ones(len(data_X_syn), dtype=torch.bool)
        is_generated_real = torch.zeros(len(data_X_real), dtype=torch.bool)

        data_mixed_X = torch.cat((data_X_real, data_X_syn), dim=0)
        data_mixed_Y = torch.cat((data_Y_real, data_Y_syn), dim=0)
        is_syn = torch.cat((is_generated_real, is_generated_syn), dim=0)
        trainset_mixed = TensorDataset(data_mixed_X, data_mixed_Y, is_syn_mixed)

        alpha= num_samples / len(trainset) # Adjusting for different dataset sizes, if needed. 
        return gen_dataset, alpha
    ...
    def combined_loss(self, criterion, D_syn_batch: torch.Tensor, D_real_batch: torch.Tensor, combine_config: Optional[Dict], device) -> torch.Tensor:
        ...
        # Probably it's better to combine Datasets and add a flag to indicate whether real or not
        ...
        if len(D_syn_batch) != len(D_real_batch):
            raise ValueError("D_syn_batch and D_real_batch must have the same number of samples for this combine method.")

        
        g_loss_attack = criterion(D_syn_batch, D_real_batch).to(device)
