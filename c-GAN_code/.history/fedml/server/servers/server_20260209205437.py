...
class Server:
    def __init__(
        ...
        *, 
        ...
        self.filters = create_filter(user_configs=user_configs)
    ...
    def fit_round(self, server_round: int):
        ...
        for k,v in self.filters.items():
            submitted_fs = {
                k: self.executor.submit(v.server_tasks, self.parameters, server_round)
            }
        finished, _ = concurrent.futures.wait(submitted_fs.get("GAN_ATTACK", set()), timeout=None)
