...
class MaliciousGenerator(GenerativeFilter):
    def __init__(self, gen_configs, train_configs, filter_configs, skip_rounds = 1) -> None:
        ...
        # self.dis_model = load_model(model_configs=dis_configs) # model class is to be provided by the server during training
    ...
    def server_tasks(self, model, server_round: int) -> None: #server task inputs model instead of global weights
