...
apptainer exec --nv --env PYTHONPATH=${code_path} $environment_path $FEDN_COMMAND
...
# See results 
    ...
    #apptainer exec --nv "/cephyr/users/garciafe/containers/fl_env_v5.sif" python -c "import numpy as np; npz=np.load('base_modified.npz'); print(npz['results'])"