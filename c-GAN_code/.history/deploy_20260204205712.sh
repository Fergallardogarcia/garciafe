...
FEDN_COMMAND="python client/client_runner.py "--num-gpus" ${FEDN_API_URL} --token ${FEDN_TOKEN} --name client-${current_proc_num} --data-path ${client_data_path}/$((${current_proc_num}+1))/${client_data_file}"
