        ...
        for epoch in range(epochs):
            for data in trainloader:
                ...
                def _mask() -> None:
                    ...
                    sign_vector = attack_payload.perturbation_sign
                ...
                def _project_perpendicular(pre_weights) -> None:
                    update = model.get_weights() - pre_weights
                    projected_update = update - torch.dot(update, correction_direction) / correction_direction.dot(correction_direction).clamp_min(1e-12) * correction_direction
                    model.set_weights(pre_weights + projected_update)
                    # -------------------------------
                # _project_perpendicular(pre_weights)
