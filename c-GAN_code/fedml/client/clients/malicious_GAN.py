...
class GanMaliciousClient(HonestClient):
    ...
    def generate_D_mixed(
        ...
        is_real = torch.cat((is_real_real, is_real_syn), dim=0)
...
def train_mixed_data(
    ...
    :param alpha: The ratio of synthetic data to real data, used to adjust the loss for the synthetic data.
    :returns: None.
    """
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(trainloader):
            ...
            loss = torch.where(is_real, -alpha*0*loss, loss) # include alpha = num_samples/len(trainset) to correct for dataset size strngth. now 1:1
