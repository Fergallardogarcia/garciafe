...
def create_filter(
    ...
    if user_configs["SERVER_CONFIGS"]["SERVER_TYPE"] == "FILTER" and mode == "DEFENSE":
        ...
        # Coordinated PERMUTER attack: reuse the GAN_attack server-side orchestrator,
        # but only to broadcast a shared permutation seed to all malicious clients.
        # It is created in a lightweight "coordinated_permuter" mode that builds no
        # generator/discriminator and trains/evaluates no GAN. COORDINATION may be an
        # int N (shared permutation for N-round cycles) or "all" (whole run); None
        # means independent per-client/round and needs no server-side component.
        if attack_type == "PERMUTER":
            from fedml.client.clients.malicious_permuter import parse_coordination
            permuter_cfg = (
                user_configs["EXPERIMENT_CONFIGS"]["MAL_HYPER_PARAM"].get("PERMUTER_CONFIG", {}) or {}
            )
            coordination = parse_coordination(permuter_cfg.get("COORDINATION", None))
            if coordination is not None:
                from fedml.attacks import GAN_attack
                return GAN_attack(
                    coordinated_permuter=True,
                    coordination=coordination,
                    permutation_seed=permuter_cfg.get("SEED", 0),
                    skip_rounds=user_configs["EXPERIMENT_CONFIGS"]["MAL_HYPER_PARAM"]["ATTACK_ROUND"],
                )

        # Any other attack (independent PERMUTER, SIGNFLIP, RANDOM, ...) has no
