...
class Server:
    def __init__(
        ...
        *, 
        ...
        run_devices = [user_configs["SERVER_CONFIGS"]["FILTER_CONFIGS"]["HYPER_PARAM"]["TRAIN_GAN_PARAMS"]["DEVICE"],
        ...
        self.attack = create_filter(user_configs=user_configs, mode="ATTACK", device=run_devices[1])
    ...
    def fit_round(self, server_round: int):
        ...
        self.attack.server_fit_round_before(
            global_parameters=self.parameters,
            server_round=server_round,
            executor=self.executor,
            client_manager=self.client_manager,
        ...
        if self.filter.server_fit_round_after() is not None:
            filter_round, log_filter_stats = self.filter.server_fit_round_after()
            selected_indexes, client_stats = filter_round(server_round=server_round, results=results)
